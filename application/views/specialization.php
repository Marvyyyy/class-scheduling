            ...
            <div class="modal-dialog modal-dialog-centered" role="document">
              <div class="modal-content">
                ...
                <div class="modal-body">
									<div class="form-group">
										<label for="exampleInputEmail1">Specialization</label>
										<input type="text" class="form-control" id="specialization_edit" name="specialization" placeholder="Enter specialization">
										<span class="text-danger" id="specialization_error_edit"></span>
									</div>
                </div>
                <div class="modal-footer">
                  <button type="button" class="btn btn-outline-primary" data-dismiss="modal">Cancel</button>
  ...
  <script type="text/javascript">
    $(document).ready(function () {
      ...
      $('#dataTableHover').DataTable(); // ID From dataTable with Hover
          ...
          "responsive": true,
					"ajax": {
						"url": "<?php echo base_url(); ?>specialization/fetch_alldata",
						"type": "POST",
						"dataSrc": ""
					},
					// dom: 'Bfrtip',
		...
		function edit(specialization_id){
				$.ajax({
					url: "<?php echo base_url('specialization/fetch_singledata/'); ?>",
					method:"POST",
					data:{idd:specialization_id},
					dataType:"json",
					success:function(response)
					{
						$('#editID').val(response.specialization_id);
						$('#specialization_edit').val(response.specialization_title);
						$('#editModal').modal({
							backdrop:"static",
							keyboard:false
						});
		...
		function deleteBtn(specialization_id){
				$.ajax({
					url: "<?php echo base_url('specialization/fetch_singledata/'); ?>",
					method:"POST",
					data:{idd:specialization_id},
					...
					success:function(response)
					{
						$('#deleteID').val(response.specialization_id);
						$('#deleteModal').modal({
							backdrop:"static",
							keyboard:false
						});
	...
	<script type="text/javascript">
		$(document).ready(function() {
			$('#add_form').on('submit', function(e) {
				e.preventDefault();
				$.ajax({
					url: "<?php echo base_url()?>specialization/add_data",
					method: "post",
					data: $(this).serialize(),
					success: function(response) {
						rs = JSON.parse(response);
						if(rs.error === true) {
							$('#specialization_error').html(rs.specialization_error);
			...
			$('#edit_form').on('submit',function(e){
					...
					$.ajax({
						...
						success: function(response) {
							rs = JSON.parse(response);
							if(rs.error === true) {
							$('#specialization_error_edit').html(rs.specialization_error);
							} else if(rs.error === false) {
			...
			$('#delete_form').on('submit',function(e){
					e.preventDefault();
					$.ajax({
						url: "<?php echo base_url()?>specialization/delete_data",
						method : "post",
						data : $(this).serialize(),
						success: function(response) {
